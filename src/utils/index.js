...
export function getDateStr(AddDayCount, format = 'yyyy-MM-dd') {
  const dd = new Date()
...
// 获得两个日期之间相差的天数
export function getDays(date1, date2, split = /[-|' ']/) {
...
export function stringify(obj) {
  ...
  const str = JSON.stringify(obj, (key, value) => {
    if (typeof value === 'object' && value !== null) {
      if (cache.indexOf(value) !== -1 || key === 'context') {
        // 移除
        return
      }
      // 收集所有的值
